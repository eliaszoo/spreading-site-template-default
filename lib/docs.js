...
async function getSlugs(project, isPreview) {
  ...
  for (const version of versions) {
    ...
    for (const collectionGroup of collectionGroups) {
      ...
      // 开始将分组下的uri进行组合
      const slug = isPreview ? [projectName, PREVIEW_KEY, version] : [projectName, version]
      ...
      for (const collection of groupCollections) {
        ...
        slugs = slugs.concat(traverseToc(collection.toc, [...slug, ...languageSlug, ...platformSlug], structure.id, version, isPreview))
      }
    }
  }
...
export async function readDoc(slug) {
  ...
  const version = isPreview ? slug[2] : slug[1]
  const mdxContent = await getMdxContent(projectName, version, slug[slug.length - 2], isPreview)
